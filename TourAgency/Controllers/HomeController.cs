...
public class HomeController : Controller
{
    ...
    public async Task<IActionResult> Index([FromQuery] bool ordered = false, [FromQuery] bool overflow = false)
    {
        ...
        var enrichedToursList = new List<TourEnriched>();

        foreach (var tour in toursList)
        {
            var isHot = (tour.StartDate.ToDateTime(TimeOnly.MinValue) - DateTime.Now).Days < 14;
            ...
            var enriched = new TourEnriched
            {
                Id = tour.Id,
                Deleted = tour.Deleted,
                EndDate = tour.EndDate,
                IsHot = isHot,
                Leftover = tour.Leftover,
                Name = tour.Name,
                Price = (int)(tour.Price * discount),
                StartDate = tour.StartDate,
                OriginalPrice = tour.Price
            };
            
            enrichedToursList.Add(enriched);
        }
        ...
        ViewBag.ToursList = enrichedToursList;
